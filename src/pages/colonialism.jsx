
//let links

function Colonialism()
{
    return(
        <div class="container">
            <div class="row">
                <div class="col">
                </div>

                <div class="col-8">
                    <h6 class="h6v2 pt-4 pb-2" style={{textAlign:'center'}}> INTERNATIONAL APIDA NEWS </h6>

                    <h2 class="h2v2 py-4" style={{textAlign:'center'}}> The Crown Jewels and Colonialism </h2>

                    <h6 class="h6v4 pb-2" style={{textAlign:'center'}}>
                        BY: BRIDGET ZHANG
                    </h6>

                    <h6 class="h6v3 pb-4" style={{textAlign:'center'}}>
                        October 4, 2022
                    </h6>

                    <div style={{display:'flex', alignItems:'center'}}>
                        <div style={{width:'100%', backgroundColor:'black', height:'5px'}}></div>
                    </div>

                    <div class="py-2">
                    </div>

                    <p class="pv2 py-2">
                        Queen Elizabeth II died on September 8, 2022, marking the end of a 70-year reign. As a symbol of the British monarchy, her legacy cannot be separated from that of the colonial empire. This is seen most obviously through the precious stones that she wore, many of which were taken from former colonies. Since her death, there have been renewed calls for the jewels to be returned to their rightful owners.
                    </p>
                    <p class="pv2 py-2">
                        As the Queen lay in state, the Imperial State Crown, the Sovereign’s Sceptre with Cross, and the Sovereign’s Orb rested upon her coffin. These three items are sacred objects that are central to the Crown Jewels collection and part of the regalia used for her coronation ceremony in 1953. Both the sceptre and the crown are set with pieces cut from the Cullinan diamond, known to be the largest gem-quality diamond that has ever been found. The Sovereign’s Sceptre holds the 530-carat Cullinan I, while the Imperial State Crown is adorned with the 317-carat Cullinan II. 
                    </p>
                    <p class="pv2 py-2">
                        The Cullinan I and Cullinan II are also known respectively as the Great Star of Africa and the Second Star of Africa. The original stone was first found in 1905 in a private mine located in South Africa’s old Transvaal province. The owner of the mine was South African diamond magnate Thomas Cullinan, who the gemstone is also named after. Two years after it was discovered, the Transvaal government, which was under British rule, purchased the diamond and presented it to King Edward VII. While claimed to have been given as a gift, it could arguably be considered an illegitimate transaction performed under colonial authority.
                    </p>
                    <p class="pv2 py-2">
                        The Imperial State Crown is also set with various other gemstones that are believed to be of Asian origin including territories encompassed by present-day Afghanistan, Myanmar, Sri Lanka, Tajikistan, and Kashmir. These gemstones include the Black Prince’s Ruby, the St. Edward’s Sapphire, and the Stuart Sapphire.
                    </p>
                    <p class="pv2 py-2">
                        Another crown in the collection, the Queen Mother’s Crown also holds a very controversial gemstone. The 105-carat Koh-i-Noor diamond is the centerpiece of this crown and has long been embroiled in an ownership dispute. While the British monarchy says that the jewel was obtained legally, four countries, namely India, Pakistan, Afghanistan, and Iran, claim that it was stolen and belongs to them.
                    </p>
                    <p class="pv2 py-2">
                        The Koh-i-Noor went through a tumultuous history – first known to have adorned Mughal Emperor Shah Jahan’s jeweled Peacock Throne. It was looted by Persian ruler Nader Shah when he invaded in 1739 and would remain outside of India for 70 years, changing owners repeatedly in the territory that would later become modern-day Afghanistan. Eventually, Sikh ruler Ranjit Singh obtained it, and the diamond became a desirable symbol of power. After the British annexed Punjab in 1849, they forced his descendant, 11-year-old Duleep Singh, to sign a treaty that gave away both the gemstone and all claim to rulership.  
                    </p>

                    <div class="py-5">
                    </div>

                    <h4 class="h4v2 py-2" style={{textAlign:'center'}}>
                        SOURCES
                    </h4>

                    <a href="https://www.hrp.org.uk/tower-of-london/history-and-stories/the-crown-jewels/#gs.ewqeft">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://www.hrp.org.uk/tower-of-london/history-and-stories/the-crown-jewels/#gs.ewqeft
                        </p>
                    </a>
                    <a href="https://www.vice.com/en/article/bvm7ad/elizabeth-ii-colonisation-kohinoor-india-pakistan">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://www.vice.com/en/article/bvm7ad/elizabeth-ii-colonisation-kohinoor-india-pakistan
                        </p>
                    </a>
                    <a href="https://www.smithsonianmag.com/history/true-story-koh-i-noor-diamondand-why-british-wont-give-it-back-180964660/">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://www.smithsonianmag.com/history/true-story-koh-i-noor-diamondand-why-british-wont-give-it-back-180964660/
                        </p>
                    </a>
                    <a href="https://www.bbc.com/news/uk-england-62906194">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://www.bbc.com/news/uk-england-62906194
                        </p>
                    </a>
                    <a href="https://www.cnn.com/style/article/great-star-of-africa-diamond-intl-lgs/index.html">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://www.cnn.com/style/article/great-star-of-africa-diamond-intl-lgs/index.html
                        </p>
                    </a>
                    <a href="https://internetstones.com/news/black-prince-ruby/">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://internetstones.com/news/black-prince-ruby/
                        </p>
                    </a>
                    <a href="https://internetstones.com/news/st-edwards-sapphire/">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://internetstones.com/news/st-edwards-sapphire/
                        </p>
                    </a>
                    <a href="https://internetstones.com/news/stuart-sapphire/">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://internetstones.com/news/stuart-sapphire/
                        </p>
                    </a>

                </div>

                <div class="col">
                </div>
            </div>
        </div>
        
        
    );

}
export default Colonialism;