

function Monterey()
{
    return(
        <div class="container">
            <div class="row">
                <div class="col">
                </div>

                <div class="col-8">
                    <h6 class="h6v2 pt-4 pb-2" style={{textAlign:'center'}}> LOCAL APIDA NEWS </h6>
                    
                    <h2 class="h2v2 py-4" style={{textAlign:'center'}}> On Lunar New Year’s Eve 2023, a Mass Shooting in Monterey Park (a predominantly Asian American community) killed 10.
                    </h2>

                    <h6 class="h6v4 pb-2" style={{textAlign:'center'}}>
                        BY: YI-ANN
                    </h6>

                    <h6 class="h6v3 pb-4" style={{textAlign:'center'}}>
                        January 22, 2023
                    </h6>

                    <div style={{display:'flex', alignItems:'center'}}>
                        <div style={{width:'100%', backgroundColor:'black', height:'5px'}}></div>
                    </div>

                    <div class="py-2">
                    </div>

                    <p class="pv2 py-2">
                        On a night meant to be full of joy and ushering in luck and fortune, Lunar New Year festivities in Monterey Park faced a devastating tragedy. According to the Los Angeles Police Department, a gunman opened fire on Saturday night and killed ten people, wounding ten more.  
                    </p>
                    <p class="pv2 py-2">
                        As of now, details about the shooter are limited and not public. The gunman is still at large, and the second day of the Lunar New Year festival in Monterey Park has been canceled.
                    </p>
                    <p class="pv2 py-2">
                        Monterey Park is a predominantly Asian American community, with two-thirds of the population identifying as such. The AAPI community also makes up more than half of Monterey Park’s neighboring city, Alhambra.
                    </p>
                    <p class="pv2 py-2">
                        This shooting has left the Asian American community deeply saddened and angry. Lunar New Year is one of the most joyous occasions of the year for our community. On a day when we should be coming together to celebrate, many of us are spending it mourning instead.
                    </p>
                    <p class="pv2 py-2">
                        DAY USC is devastated for the lives lost last night and are heartbroken for the families who were impacted and are grieving. We are also incredibly angry that violence against the Asian American community continues to persist. Our community has fought hard to shine a light on the tragedies and attacks against our people in the past few years, and this shooting happening on a day symbolic of family, safety, and celebration is equally shocking as it is appalling.
                    </p>
                    <p class="pv2 py-2">
                        With more details of the shooting to come, we urge you to check in on your Asian American friends, and continue to uplift APIDA voices and stories. We hope the media will report on this with accuracy, compassion, and conviction–it would be a disservice to do so otherwise. And finally, we encourage everyone to reflect on where this mass shooting fits into the bigger picture of public safety, gun violence, and the wellbeing of all marginalized communities who have been constantly and consistently fighting against such hate and violence.
                    </p>

                    <div class="py-5">
                    </div>

                    <h4 class="h4v2 py-2" style={{textAlign:'center'}}>
                        SOURCES
                    </h4>

                    <a href="https://www.nytimes.com/live/2023/01/22/us/shooting-monterey-park-california#shocked-saddened-angered-reactions-to-the-mass-shooting-in-monterey-park">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://www.nytimes.com/live/2023/01/22/us/shooting-monterey-park-california#shocked-saddened-angered-reactions-to-the-mass-shooting-in-monterey-park
                        </p>
                    </a>
                    <a href="https://www.nbcnews.com/news/us-news/mass-shooting-monterey-park-california-rcna66877">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://www.nbcnews.com/news/us-news/mass-shooting-monterey-park-california-rcna66877
                        </p>
                    </a>
                    <a href="https://www.montereypark.ca.gov/669/Lunar-New-Year-Festival">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://www.montereypark.ca.gov/669/Lunar-New-Year-Festival
                        </p>
                    </a>
                    <a href="https://www.cbsnews.com/losangeles/live-updates/the-latest-details-on-the-monterey-park-mass-shooting/">
                        <p class="pv2" style={{textAlign:'center'}}>
                        https://www.cbsnews.com/losangeles/live-updates/the-latest-details-on-the-monterey-park-mass-shooting/
                        </p>
                    </a>
                </div>

                <div class="col">
                </div>
            </div>
        </div>
        
        
    );

}
export default Monterey;